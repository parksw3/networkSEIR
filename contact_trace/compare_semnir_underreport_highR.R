...
load("seminr_mle_highR.rda")

reslist[[1]] <- rlist[[1]]

load("seminr_mle_underreport_highR.rda")
...
g1 <- ggplot(df %>% filter(param=="mean", mean < 500)) +
...
g3 <- ggplot(df %>% filter(param=="R", likelihood=="full", mean<200)) +
...
ggsave("compare_seminr_underreport_highR.pdf", gfinal, width=8, height=6)
