...
df <- mlelist %>%
    bind_rows(.id="type") %>%
    mutate(type=factor(type, labels=c("poisson", "epidemic"))) %>%
    rename(likelihood=method)

g1 <- ggplot(df %>% filter(param=="mean", type=="poisson")) +
...
g2 <- ggplot(df %>% filter(param=="shape", type=="poisson")) +
...
g3 <- ggplot(coverdf %>% filter(type=="poisson")) +
...
g4 <- g1 %+% (df %>% filter(param=="mean", type=="epidemic")) +
    ggtitle("epidemic") +
    theme(
        legend.position="none"
    )

g5 <- g2 %+% (df %>% filter(param=="shape", type=="epidemic"))

g6 <- g3 %+% (coverdf %>% filter(type=="epidemic"))
...
ggsave("compare_gamma.pdf", gfinal, width=8, height=6)
