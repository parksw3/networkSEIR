...
\vspace{-1 cm}
\colorbox{white}{
\includegraphics[width=0.78\textwidth]{../fig/diagram.pdf}
}
\vspace{-0.5 cm}
\end{center}
...
Model parameters: $\mathcal{R} = 6$, mean latent,  infectious periods of 1, 2 days.
\end{block}
}
\end{minipage}
\end{beamercolorbox}
...
(Left) Observed GIs from simulations on strictly local networks (per neighbor contact rate of 0.2 $\text{time}^{-1}$). These do not match the known intrinsic GI, but do match the calculated spatial correction. 
...
\begin{figure}
\vspace{-0.5 cm}
\includegraphics[width=0.95\textwidth]{../fig/corrected_GI.pdf}
\vspace{-0.5 cm}
\end{figure}
\textbf{Reproductive number predicted by using the corrected GI distributions (effective GI) matches empirical estimate of the reproductive number.} 
(Left) Contact tracing data provides longer GI ($\approx$ 1.1 days; histogram) than predicted GI ($\approx$ 1 day; solid curve - ``observed'') from local correction.
(Right) Effective GI ($\approx$ 2.1 days; histogram), obtained by temporally correcting contact-tracing data, is shorter than locally corrected GI ($\approx$ 2.4 days; solid curve - ``local'') or intrinsic GI (3 days; not shown).
...
Temporally correcting contact tracing data is sensitive to long GIs and can overestimte $\R$.
Stochastic SEIR model simulated on an empirical network \cite{leskovec2016snap}.
\end{block}
}
\end{minipage}
\end{beamercolorbox}
