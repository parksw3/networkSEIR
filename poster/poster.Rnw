...
This correction requires an estimate of degree distribution, and either an explicit model structure (like SEIR) or information about individual-level variation in infectiousness.
\end{block}
}
\end{minipage}
\end{beamercolorbox}
...
Local correction of intrinsic GIs \cite{trapman2016inferring} is difficult and approximate. 
\end{block}
}
\end{minipage}
\end{beamercolorbox}
