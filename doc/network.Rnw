...
\subsection*{Intrinsic generation interval}

This section serves to introduce notation from previous work [CITE].
Let $K(t)$ be the infection kernel. The reproduction number is defined as
$$
\RR = \int_0^\infty K(t).
$$
...
\subsection*{The observed generation interval through time}
...
\begin{equation}
g_t(\tau)= \frac{\RR \int_\tau^t i(s-\tau) g(\tau) S(s) ds}{\RR \int_0^t \int_x^t i(s-x) g(x) S(s) ds dx}.
\end{equation}
...
Then, we have
\begin{equation}
g_t(\tau) = \frac{\RR \int_\tau^t i(s-\tau) g(\tau) S(s) ds}{\int_0^t i(s) ds}.
\end{equation}
For convenience, we ignore normalizing constants and write
\begin{equation}\label{eq:obsg}
g_t(\tau) \propto g(\tau) \int_{0}^t i(s-\tau) S(s) ds.
\end{equation}

\subsection*{Recovering intrinsic generation interval}
