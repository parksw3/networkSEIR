...
\definecolor{intc}{RGB}{179, 211, 286}
...
\node(int)[bigcompartment, above=2cm of local, text width=5cm, bottom color=intc, fill opacity=0.7, text opacity=1]{
    \textbf{Intrinsic GI}
    \footnotesize{
        \begin{itemize}
        \item patient-based 
...
\node(eff)[bigcompartment, right=3.4cm of local, text width=6cm, bottom color=effc, fill opacity=0.7, text opacity=1]{
    ...
    \item gives the correct link between $r$ and $\R$
    \end{itemize}
};
...
\draw[none] (obs) -- node [text width=5cm, midway, right] {\footnotesize{weight observed periods by $\exp(r\tau)$}} (eff);

\draw[<->, thick, dotted] (obs) -- node [text width=2cm, above]{\textbf{Homogeneous\\ assumption}}(int);
\draw[none] (obs) -- node [text width=2cm, below]{Temporal correction}(int);
